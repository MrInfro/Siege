...
msg(sender, ChatColor.WHITE + "* (admin only) /sm reload - config reload", new Object[0]);
msg(sender, ChatColor.WHITE + "* (admin only) /sm setSiege cityNumber hour minute", new Object[0]);
msg(sender, ChatColor.WHITE + "* (admin only) /sm reschedule - reschedules SM (emergency)", new Object[0]);
msg(sender, ChatColor.WHITE + "* (admin only) /sm startSiege cityNumber - manual start & end", new Object[0]);
msg(sender, ChatColor.WHITE + "* (admin only) /sm endSiege cityNumber - force siege end", new Object[0]);
return true;
}

if (args[0].equalsIgnoreCase("startSiege") && (hasPerm(sender, "SiegeMode.use"))) {
this.calendar.setTime(this.date);
int time_in_seconds = this.calendar.get(11) * 3600 + this.calendar.get(12) * 60 + this.calendar.get(13);
getServer().getScheduler().scheduleSyncRepeatingTask(this, new Runnable() {
public void run() {
SiegeMode.this.runCommand(args[1], "startCity");
}
}, (time_in_seconds + 60) * 20L, 1728000L);
getServer().getScheduler().scheduleSyncRepeatingTask(this, new Runnable() {
public void run() {
SiegeMode.this.runCommand(args[1], "endCity");
}
}, (time_in_seconds + 7260) * 20L, 1728000L);
msg(sender, ChatColor.RED + "[SiegeMode] " + ChatColor.WHITE + "Manual start siege start on city " + args[1] + "in 1min.", new Object[]);
log.log(Level.INFO, "[SiegeMode] Manual start of siege event on City" + args[1]);
return true;
}
if (args[0].equalsIgnoreCase("endSiege") && (hasPerm(sender, "SiegeMode.use"))) {
this.calendar.setTime(this.date);
int time_in_seconds = this.calendar.get(11) * 3600 + this.calendar.get(12) * 60 + this.calendar.get(13);
getServer().getScheduler().scheduleSyncRepeatingTask(this, new Runnable() {
public void run() {
...
}, time_in_seconds * 20L, 1728000L);
msg(sender, ChatColor.RED + "[SiegeMode] " + ChatColor.WHITE + "Manual end of siege on city " + args[1], new Object[]);
log.log(Level.INFO, "[SiegeMode] Manual end of siege event on City" + args[1]);
return true;
}

if (args[0].equalsIgnoreCase("reschedule") && (hasPerm(sender, "SiegeMode.use"))) {
startSchedule();
log.log(Level.INFO, "[SiegeMode] Emergency reschedule activated! God help us all...");
return true;
}

if ((args[0].equalsIgnoreCase("setSiege")) && (hasPerm(sender, "SiegeMode.use"))) {
if ((0 < Integer.parseInt(args[1]) >= 7) && (0 <= Integer.parseInt(args[2]) >= 21) && (0 <= Integer.parseInt(args[3]) >= 59) && !(getConfig().getInt("SiegeSchedule.startCity" + args[1] + ".Day") == getDayOfWeek())) {
try {
...
msg(sender, ChatColor.RED + "[SiegeMode] u fail! spatne napsane nebo siege uz dnes...", new Object[0]);
